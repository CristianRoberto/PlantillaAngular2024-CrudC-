...
export class TableService {
  ...
  public filterDataByProps(
    ...
    return dataToFilter.filter((row) =>
      propsToFilter.some((prop) =>
        row[prop].toString().toLowerCase().includes(textToFilter.toLowerCase())
      )
    );
  }
  /**
  ...
  public formatDataToTable<T>(
    ...
    if (!dataToFormat) return []; // Verificar si dataToFormat es undefined o null
    return dataToFormat.map((row) => ({
      ...row,
      key: row[keyName]?.toString(), // Usar optional chaining para evitar errores si keyName es undefined
    }));
  }
