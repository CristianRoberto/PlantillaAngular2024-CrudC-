...
export class InputComponent implements OnInit {
  ...
  public onChangeInput(valueInput: any) {
    ...
    switch (this.type) {
      ...
      case "color":
        {
          const inputColorEvent: Event = valueInput;
          const inputColorTarget = inputColorEvent.target as HTMLInputElement;
          value = inputColorTarget.value;
        }
        break;
      case "select":
        {
  ...
  public keypress(event: KeyboardEvent) {
    ...
    if (!preventPress && this.allowedKeys?.length > 0) {
      ...
      const isAllowedDescription = this.inputService.isAllowedDescription(inputKey);
      preventPress =
        ...
        (this.allowedKeys.includes("isAllowedDescription") && !isAllowedDescription) ||
